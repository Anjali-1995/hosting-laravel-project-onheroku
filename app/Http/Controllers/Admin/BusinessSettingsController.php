...
class BusinessSettingsController extends Controller
{
    ...
    public function restaurant_setup(Request $request)
    {
        ...
        if (!empty($request->file('logo'))) {
            ...
            if (!Storage::disk('public')->exists('restaurant')) {
                Storage::disk('public')->makeDirectory('restaurant');
            }
            if (isset($curr_logo) && Storage::disk('public')->exists('restaurant/' . $curr_logo['value'])) {
                Storage::disk('public')->delete('restaurant/' . $curr_logo['value']);
            }
            $note_img = Image::make($request->file('logo'))->stream();
            Storage::disk('public')->put('restaurant/' . $image_name, $note_img);
        } else {
    ...
    public function update_fcm_messages(Request $request)
    {
        ...
        DB::table('business_settings')->updateOrInsert(['key' => 'out_for_delivery_message'], [
            'value' => json_encode([
                'status' => $request['out_for_delivery_status'] == 0 ? 1 : 0,
