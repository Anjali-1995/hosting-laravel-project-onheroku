...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        //$token= DeliveryMan::select('fcm_token')->where(['id' =>$delivery_man_id ])->first();
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
       try {
        ...
        $d_man= DeliveryMan::find($delivery_man_id);
        if($d_man==null){
            Toastr::failed('No Such Delivery Man Found!');
        return response()->json([], 401);
        }
        ...
        $user = $order->customer;
        //dd($user);
        $customer_fcm_token = $user->cm_firebase_token;
        ...
        try {
            $data = [
                ...
                'description' => 'Order id '.$order->id.' has been assigned to you',
                'order_id' => $order->id,
                'image'  => '',
            ];
