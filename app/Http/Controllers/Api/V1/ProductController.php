...
class ProductController extends Controller
{
    public function get_latest_products(Request $request)
    {
        ...
        if(\count($product_ids)){
                ...
                $products['products']  = Product::active()->with(['reviews'])->latest()->whereIn('id',$product_ids)->paginate($request['limit'], ['*'], 'page', $request['offset'])->items();
                $products['total_size'] = count($products['products']);
        }
    ...
    public function get_searched_products(Request $request)
    {
        ...
        $products = ProductLogic::search_products($request,$request['name'], $request['limit'], $request['offset']);
    ...
    {
        if (Product::find($id)) {
            $products = ProductLogic::get_related_products($request,$id);
    ...
    {
        $product_ids = ProductLogic::find_products_of_closest_pantry($request);
        $tmp  = Product::active()->with(['reviews'])->where(['set_menu' => 1, 'status' => 1])->whereIn('id',$product_ids)->get();


        try {
            $products = Helpers::product_data_formatting($tmp, true);
            return response()->json($products, 200);
        } catch (\Exception $e) {
            return response()->json([
