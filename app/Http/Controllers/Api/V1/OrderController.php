...
class OrderController extends Controller
{
    ...
    public function payment_successful(Request $request){
        ...
        if($order->payment_method=='cash_on_delivery'){
            return response()->json(['errors'=>'please change the payment method first'],403);
        }
        if($order==null){
            return response()->json(['errors'=>'no such order found'],403);
        }
        $order->payment_status='paid';
        $order->transaction_reference=$request->get('transaction_reference');
        $order->save();
    ...
    public function place_order(Request $request)
    {
        ...
        try {
            $or = [
                ...
                'coupon_discount_amount' => $request->coupon_discount_amount == null ? 0: $request->coupon_discount_amount,
            ...
            foreach ($request['cart'] as $c) {
                ...
                $tmpCount = 0;
                if($product){
                    $tmpCount = count(json_decode($product['variations'],true));
                if ( $tmpCount> 0) {
